...
function consultarUsuarios(doc) {
    $.ajax({
        ...
        if (doc == '') {
            // Tabla
            $('#content-tabla').empty();
            $('#content-tabla').html('<table width="100%" class="table table-striped table-bordered table-hover" id="dataTableUsuario">' + '<thead>' + '<tr>' + '<th>Documento</th>' + '<th>Nombres</th>' + '<th>Apellidos</th>' + '<th>Rol</th>' + '<th>Estado</th>' + '<th>Acciones</th>' + '</tr>' + '</thead>' + '<tbody id="cuerpo">' + '</tbody>' + '</table>');
            $.each(result, function(index, row) {
                if (row.documento != doc) {
                    if (row.estado != 2) {
                        $('#cuerpo').append('<tr class="even gradeC">' + '<td>' + row.documento + '</td>' + '<td>' + row.nombres + '</td>' + '<td>' + row.apellidos + '</td>' + '<td class="center">' + row.rol + '</td>' + '<td class="center">' + clasificarEstado(row.estado) + '</td>' + '<td class="center">' + '<button class="btn btn-primary btn-xs" onclick="mostrarInformacionUsuarioModal(' + row.documento + ')">' + '<span><i class="fas fa-edit"></i>' + '</span> Editar' + '</button>&nbsp;' + '<button class="btn btn-' + (row.estado == 1 ? 'danger' : 'success') + ' btn-xs" onclick="cambiarEstadoUsuario(' + row.documento + ')">' + '<span><i class="' + (row.estado == 1 ? 'fas fa-user-times' : 'fas fa-user-check') + '"></i>' + '</span> ' + (row.estado == 1 ? 'Desactivar' : 'Activar') + '' + '</button>' + '</td>' + '</tr>');
                    }
                }
            });
            // 
    ...
    swal({
        ...
        if (result) {
            $.ajax({
                ...
                swal('Realizado!', 'El usuario fue ' + (result == 1 ? 'desactivado' : 'Activado') + ' Correctamente', 'success', {
                    buttons: false,
                    timer: 2000
                });
                // console.log("success");
