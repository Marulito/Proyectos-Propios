...
$(document).ready(function($) {
    ...
    $('#formularioDoc').submit(function(event) { //Validacion de campos
        ...
        if ($('#accionarDocumentos').val() == 0) { //Registrar
            if ($('#userfile').val() == '') {
                ...
                swal('Alerta', 'Porfavor Carga un documento', 'warning', {
                    buttons: false,
                    timer: 2000
                });
            } else {
            ...
            if ($('#userfile').val() == '') {
                // ...
                $.post(baseurl + 'cDocumento/descargarDocumento', {
                    idDoc: $('#accionarDocumentos').val(),
    ...
    $('#accionarDescarga').click(function() {
        ...
        if ($('#contraseñaD').val() != '') {
            ...
            swal('Alerta', 'Debes ingresar tu contraseña', 'warning', {
                buttons: false,
                timer: 2000
            });
        }
...
function eliminarDocumento(name) {
    $.post(baseurl + 'cDocumento/deletDocument', {
        nombre: name
    }, function(data) {
...
function validarUsuario(password, idD) {
    $.post(baseurl + 'cDocumento/validarUsusario', {
        pass: password,
        idD: idD
    }, function(data) {
        if (data > 0) {
    ...
    $.ajax({
        ...
        success: function(dato) {
            ...
            if (dato == 1 || dato == 2) {
                ...
                swal('Realizado!', (dato == 1 ? 'El documiento fue cargado correctamente.' : 'El documento fue modificado correctamente'), 'success', {
                    buttons: false,
                    timer: 2000
                });
            }
        ...
        // console.log(error);
        swal('Error!', 'Ocurrio un error en el procedimiento', 'error', {
            buttons: false,
            timer: 2000
        });
    });
...
function cambiarEstadoContenido(idCon, idProc, event) {
    ...
    swal({
        ...
        if (result) {
            $.post(baseurl + 'cContenido/cambiarEstadoContenido', {
                idCon: idCon,
                idProceso: idProc
            }, function(data) {
                consultarContenidos(localStorage.getItem('idTipoP'), localStorage.getItem('Contenido')); //ID de tipo de proceso
                swal('Realizado!', 'Se cambio el estado correctamente.', 'success', {
                    buttons: false,
                    timer: 2000
                });
            });
        ...
        swal({
            ...
            if (result) {
                $.ajax({
                    ...
                    $modal1.modal('hide');
                    swal('Realizado!', 'La gestion fue ' + dato == 1 ? 'Registrada' : 'Modificar' + 'Correctamente', 'success', {
                        buttons: false,
                        timer: 2000
                    });
                }).fail(function() {
...
function consultarDocumentos(idDoc, idCon, acc) { //acc= 0 general, 1=editar y 2=descarga
    $.post(baseurl + 'cContenido/documentos', {
        idDoc: idDoc,
        idPro: idCon,
        accion: acc
    }, function(data) {
...
function cambiarEstado(idD) {
    ...
    swal({
        ...
        // Confirmo
        if (result) {
            $.post(baseurl + 'cDocumento/cambiarEstadoDocumento', {
                idD: idD
            }, function(data) {
                if (data == 1) {
                    swal('Realizado', 'El cambio de estado fue realizado correctamente', 'success', {
                        buttons: false,
                        timer: 2000
                    });
                    consultarDocumentos(0, localStorage.getItem('Contenido'), 0);
                } else {
                    swal('Alerta', 'El estado no se actualizo', 'warning', {
                        buttons: false,
                        timer: 2000
                    });
                }
...
function consultarContenidos(tipo, idcon) { //ID Tipo proceso y ID del proceso
    $.post(baseurl + 'cContenido/consultarContendio', {
        idCon: idcon,
        tipoP: tipo
    }, function(data) {
        var result = JSON.parse(data);
        ...
        $.post(baseurl + 'cLogin/recuperarRol', function(data) { //La plantila se deforma cuando se agregan muchos componenetes
            ...
            $.each(result, function(index, item) {
                ...
                $('#contenido').append('<div class="col-lg-3 col-md-6">' +
                    ...
                    '<div class="panel panel-' + clasificarColorContenido(Number(item.idtipo_proceso)) + '" hidden>' + '<div class="panel-heading">' + '<div class="row">' + '<div class="col-xs-3">' + '<i class="' + clasificarIconoDelContenido(Number(item.idtipo_proceso)) + '"></i>' + '</div>' + '<div class="col-xs-9 text-right">' + '<!-- Cantidad de Procesos -->' + (tipo == 1 ? '<div>' + '<!-- Editar -->' + '<a href="" data-toggle="tooltip" data-placement="top" title="Editar" onclick="mostrarModalEditar(' + item.idProceso + ',' + item.idProceso_sub + ',\'' + item.nombre_proceso + '\',event)" class="modificar" data-idcon="' + item.idProceso + '" data-idproc="' + item.idProceso_sub + '"><span><i class="fas fa-edit" style="color: #0048A2; width: 30; "></i></span></a>' +
                        // Clasificar el estado del boron
                        '<a href="" data-toggle="tooltip" data-placement="top" title="' + (item.estado_visibilidad == 1 ? 'Visible' : 'No visible') + '" onclick="cambiarEstadoContenido(' + item.idProceso + ',' + item.idProceso_sub + ',event)" class="cambiarEstado" data-idcon="' + item.idProceso + '" data-idproc="' + item.idProceso_sub + '"><span><i ' + (item.estado_visibilidad == 1 ? 'class="fas fa-eye " style="color:#12FF00; width: 30;"' : 'class="fas fa-eye-slash" style="color:#FF0000; width: 30;"') + '></i></span></a>' + '</div>' : '<br>') + '<!-- Numero de documentos que tiene este componenete -->' + '<div class="huge">' + item.cantidad + '</div>' + '<div>' + item.nombre_proceso + '</div>' + '</div>' + '</div>' + '</div>' + '<a href="#" onclick="cambiarContenidoVista(' + item.idtipo_proceso + ',' + item.idProceso + ',event,' + '\'' + item.nombre_proceso + '\')">' + '<div class="panel-footer">' + '<span class="pull-left">Detalles</span>' + '<span class="pull-right"><i class="fa fa-arrow-circle-right"></i></span>' + '<div class="clearfix"></div>' + '</div>' + '</a>' + '</div>' + '</div>');
                // Fin del contenido-...
                con++;
